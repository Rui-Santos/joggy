...
function socketJoin(message) {
    ...
    else if (message.game === 'ofcp') type = Views.OfcpMachineView
    else throw new Error('machine view type not found for ' + message.game)

    debug('view for machine created. making model')

    if (!type) throw new Error('type not found for machine')
    if (!type.model) throw new Error('model for type not found')
