...
, MenuView = module.exports = Backbone.View.extend({
    ...
    depositClicked: function() {
        ...
        , html = '<p>Send bitcoins to <a href="bitcoin:' + address + '?label=play.luckco.in">' +
            address +
            '</a></p>' +
            '<p>Minimum 0.0001 BTC (one credit). It can take some minutes before you get credited</p>'
    ...
    withdrawClicked: function() {
        ...
        if (app.user.get('balance') <= 10 * 1e5) {
            return alertify.alert('minimum withdraw is 11 credits to avoid abuse, sorry!')
        }
