...
, debug = require('debug')('joggy:Kitty')
, app = require('../../app')
, drawPositions = [
    { x: 50, y: 73 },
    { x: 130, y: 73 },
...
Kitty.prototype.init = function(canvas) {
    ...
    this.redraw = true
    this.$canvas = $(canvas)
    this.onCanvasClick = _.bind(this.onCanvasClick, this)
    this.$canvas.on('click', this.onCanvasClick)

    debug('loading fonts...')

    cu.loadAssets({ symbols: '/media/kitty/symbols.png?1357578782512' }, function(err, a) {
...
Kitty.prototype.dispose = function() {
    this.$canvas.off('click', this.onCanvasClick)
