...
, BjMachineView = module.exports = MachineView.extend({
    ...
    assetsLoaded: function() {
        ...
        _.each(model.boxes, function(boxState, bi) {
            ...
            box.bet.chips = boxState.bet || 0
            box.player = boxState.player || null
        })
    ...
    toggleActions: function() {
        ...
        _.each(this.table.boxes, function(box) {
            box.sit.setVisible(self.state === 'betting' && app.user && !box.player)
        })
    ...
    onSit: function(packet, cb) {
        ...
        box.setPlayer(packet.player)
        box.sit.setVisible(false)
    ...
    onClickBox: function(e) {
        ...
        if (this.state === 'betting' && !box.player && app.user) {
            this.send('sit', { box: e.box })
        } else if (this.state === 'betting' && app.user && box.player === app.user.id) {
