...
, Box = module.exports = function(assets) {
    ...
    this.sit.setAttrs({
        x: -this.sit.getWidth() / 2,
        y: -this.sit.getHeight() / 2
    })

    this.node.add(this.sit)
}

Box.prototype.setPlayer = function(value) {
    this.player = value
}
...
Box.prototype.addHand = function() {
    ...
    hand.node.setId(this.node.getId() + '-hand-' + this.splits)
    this.hands[this.splits] = hand
...
Box.prototype.layout = function() {
    ...
    var setup = layouts[this.hands.length]
...
Box.prototype.split = function(shi, cards) {
    this.splits++
...
Box.prototype.discard = function(cb) {
    ...
    async.forEach(this.hands, function(hand, next) {
        hand.discard(next)
    }, function() {
        self.splits = null
        self.hands = []
