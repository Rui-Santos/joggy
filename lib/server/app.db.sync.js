...
module.exports = function(method, model, options) {
    ...
    if (method === 'read') {
        services.db.collection(model.urlRoot).findOne({
            _id: model.id
        }, function(err, doc) {
            if (err) {
        ...
        services.db.collection(model.urlRoot).update({
            _id: model.id
        }, {
            $set: j
        }, function(err) {
        ...
        services.db.collection(model.urlRoot).insert(j, function(err, doc) {
            if (err) {
                if  (options.error) return options.error(err)
                throw err
            }
            ...
            if (!model.id) throw new Error('failed to obtained _id after create')
