...
function Site(config) {
    ...
    if (services.config.BTC && services.config.zeroConf) {
        self.zeroConf = new ZeroConf()
    }

    if (services.config.disableCompetitions !== true) {
...
Site.prototype.joinMachineById = function(client, id) {
    Machine.find(id, function(err, machine) {
        if (err) return console.error(err)
        if (!machine) {
            return client.socket.emit('error', 'machine ' + id + ' not found')
            debug('user tried to join machine ' + id + ' which does not exist')
            return
        }

        var game = machine.model.get('game')

        if (services.config.disabledGames && ~services.config.disabledGames.indexOf(game)) {
            return client.socket.emit('error', 'game is disabled')
        }
...
Site.prototype.joinMachineByGame = function(client, game) {
    var type

    if (game === 'job') type = JobMachine
    else if (game === 'rwb') type = RwbMachine
    ...
    Machine.findOrCreate(type, client.user.model.id, function(err, machine) {
        if (err) {
            console.error('failed to make machine ' + game + ' for user ' + client.user.model.id)
            console.error(err)
...
Site.prototype.onClientJoin = function(client, packet) {
    ...
    if (packet.id) {
        this.joinMachineById(client, packet.id)
    } else {
        this.joinMachineByGame(client, packet.game)
    }
