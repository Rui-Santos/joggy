...
Machine.forUser = function(type, userId, cb) {
    ...
    services.db.collection('machines').findOne(q, function(err, doc) {
        ...
        // concurrency
        if (machines[model.id]) return machines[model.id]
...
Machine.find = function(id, cb) {
    ...
    var q = { _id: services.db.collection('machines').id(id)  }
    services.db.collection('machines').findOne(q, function(err, doc) {
        if (err) return cb(err)
        if (!doc) return cb()

        var type
        if (doc.game === 'bj') type = require('./BjMachine')
        if (doc.game === 'ofcp') type = require('./OfcpMachine')
        if (!type) throw new Error('no type for ' + doc.game)
...
Machine.prototype.sendToUser = function(user, name, message) {
    var that = this
    , clients = this.clients.filter(function(c) {
        console.log(c.user.model.id, user.model.id)
        return c.user === user
    })

    if (!clients.length) debug('trying to send to user with no clients')

    _.each(clients, function(c) {
        that.send(c, name, message)
    })
}
...
Machine.prototype.subscribe = function(client, name, fn, context) {
    ...
    var wrapper = function(message) {
        ...
        self.queue(function(leave) {
            ...
            try {
                fn(client, message, function(err) {
