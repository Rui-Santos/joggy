...
Chat.prototype.addClient = function(client) {
    ...
    var listener = this.listeners[client] = self.clientChat.bind(self, client)
...
Chat.prototype.removeClient = function(client) {
    ...
    debug('??? CHAT: REMOVE CLIENT ' + client.socket.id)
    debug('removing client ' + client.socket.id)

    if (this.listeners[client] === null) throw new Error('removeClient called multiple times for ' + client.socket.id)

    client.socket.removeListener('chat', this.listeners[client])
    debug('listener removed for client')

    if (client.user) {
        var clientCount = this.users.filter(function(u) { return u.model.id == client.user.model.id }).length

        // user is disconnecting his last client
        if (clientCount === 1) {
            this.users.splice(this.users.indexOf(client.user), 1)
            this.emit('leave', client.user)
            this.broadcast('chat:usercount', { users: this.users.length })
        }
    }
