...
Chat.prototype.clientChat = function(client, packet, callback) {
    ...
    client.user.log({ type: 'chat', message: packet.message }, function(err) {
        if (err) console.error('failed to log chat message')
    })

    var limiter = this.limiters[client.socket.id] || (this.limiters[client.socket.id] = Semaphore(7))

    limiter.take(function(leave) {
        self.broadcast('chat', _.extend(packet, { from: client.user.model.get('username') }))
        self.emit('chat', _.extend({}, packet, { client: client }))
        setTimeout(leave, 1800 * limiter.current)
    })
}
...
Chat.prototype.removeClient = function(client) {
    ...
    delete this.limiters[client.socket.id]
