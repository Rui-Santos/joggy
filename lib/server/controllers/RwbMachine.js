...
RwbMachine.prototype.onClientSpin = function(client, message, cb) {
    ...
    var reason = util.format('spin for %s credit on rwb machine %s', 1, self.model.id)

    client.user.wager(1 * 1e5, reason, function(err) {
        if (err) {
            console.error(err)
        ...
        client.user.give(payout * 1e5, reason, cb)
    })
}
