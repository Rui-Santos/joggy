...
function Sprinkles() {
    ...
    services.site.chat.on('chat', this.onChatXmasBonusCheck.bind(this))
    ...
    this.issued = services.db.get('sprinkles.bonuses.issued')
}
...
Sprinkles.prototype.onChatJoin = function(user, client) {
    ...
    this.xmasBonusCheck(user, client)
}
...
Sprinkles.prototype.xmasBonusCheck = function(user, client) {
    var self = this

    self.clientCanReceiveBonus(client, 'xmas bonus', 24 * 36e5, function(err, can) {
        if (err) throw err
        if (!can) return

        var text = util.format('MERRY XMAS %s! to get your gift, write <b>xmas bonus</b>',
            user.model.get('username'))
...
Sprinkles.prototype.onChatWelcomeBonusCheck = function(packet) {
    ...
    this.clientCanReceiveBonus(packet.client, 'welcome bonus', null, function(err, can) {
        ...
        self.issued.insert(issue, function(err) {
            ...
            user.bonus(credits * 1e5, 10 * credits * 1e5, 'welcome bonus', function(err) {
                if (err) throw err

                self.broadcast(util.format(
                    'welcome to luckoin, %s!, here is... %d CREDITS! :)',
                    user.model.get('username'),
                    credits))
            })
        })
...
Sprinkles.prototype.onChatXmasBonusCheck = function(packet) {
    ...
    this.clientCanReceiveBonus(packet.client, 'xmas bonus', 24 * 36e5, function(err, can) {
        if (err) throw err

        if (!can) {
            self.private(packet.client.user, 'you already got your present, remember? :)')
            return
        }
        ...
        self.issued.insert(issue, function(err) {
            ...
            user.bonus(credits * 1e5, 10 * credits * 1e5, 'xmas bonus', function(err) {
                ...
                self.broadcast(util.format(
                    'merry xmas %s, here is... %d CREDITS! :)',
                    user.model.get('username'),
                    credits))
            })
        })
