...
BjMachine.prototype.reset = function(cb) {
    ...
    _.each(this.model.get('boxes'), function(box, index) {
        ...
        if (box.player && box.idleSince + settings.idleEjectTime < +new Date()) {
            debug('ejecting player from box ' + index + ' for being idle too long')
            self.eject(index)
        }
    })
...
BjMachine.prototype.onClientSit = function(client, packet, cb) {
    ...
    var seatCount = _.where(this.model.get('boxes'), { player: client.user }).length
...
BjMachine.prototype.onClientEject = function(client, packet, cb) {
    ...
    if (box.player !== client.user) throw new Error('player not in box')
...
BjMachine.prototype.add = function(client, callback) {
    ...
    if (!existing) {
        this.model.get('players').push(client.user)
...
BjMachine.prototype.onClientDouble = function(client, message, callback) {
    ...
    if (hand.index !== 0 && !this.model.get('rules').doubleAfterSplit) {
    ...
    client.user.wager(hand.bet, 'Blackjack double', function(err) {
        ...
        hand.cards.push(card)
        debug(util.format('player doubles a %s to a %s', bj.pretty(card), bj.pretty(hand.cards)))
...
BjMachine.prototype.deal = function(cb) {
    ...
    this.broadcast('deal', {
        ...
        dealer: bj.isBlackjack(this.model.get('dealer')) ?
            this.model.get('dealer') :
...
BjMachine.prototype.getNextTurn = function(turn) {
    ...
    while (true) {
        ...
        var box = _.where(this.model.get('boxes'), { index: turn[0] })[0]
        ...
        var hand = _.where(box.hands, { index: turn[1] })[0]
