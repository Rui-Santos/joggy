...
function BjMachine(model, options) {
    ...
    this.ejectIdleUsersTimer = setInterval(function() {
        self.queue(function(leave) {
            self.model.get('state') === 'betting' && self.ejectIdleUsers()
            leave()
        })
    }, 5 * 1000)
}
...
BjMachine.prototype.ejectIdleUsers = function() {
    var self = this

    debug('ejecting idle users')
