...
OfcpMachine.prototype.add = function(client) {
    ...
    this.subscribe(client, 'done', this.onClientDone.bind(this))
}
...
OfcpMachine.prototype.onClientSit = function(client, packet, cb) {
    ...
    */
    ...
    if (this.model.get('spots').indexOf(spot) === 1) {
        this.deal(cb)
    } else {
        cb()
    }
...
OfcpMachine.prototype.onClientDone = function(client, packet, cb) {
    var that = this
    , spot = this.model.get('spots')[packet.spot]
    // TODO: ensure user
    // TODO: ensure setting is valid

    _.each(packet.cards, function(c) {
        spot.set[c.setting].push(c.card)
    })

    spot.unset = null

    this.broadcast('done', packet)

    // all done?
    if (_.all(this.model.get('spots'), function(s) {
        return !s.unset
    })) {
        var turnIndex = _.isNumber(this.model.get('turn')) ?
            (this.model.get('turn') + 1) % this.model.get('spots').length :
            0
        , turnSpot = this.model.get('spots')[turnIndex]
        , showdown = turnSpot.set[0].length +
            turnSpot.set[1].length +
            turnSpot.set[2].length === 13
        ...
        this.sendToUser(turnSpot.user, 'deal', {
            cards: turnSpot.unset,
            spot: turnIndex
        })

        this.model.set('turn', turnIndex)
    }
...
OfcpMachine.prototype.deal = function(cb) {
    ...
    _.each(this.model.get('spots'), function(spot, index) {
        ...
        that.sendToUser(spot.user, 'deal', {
            cards: spot.unset,
            spot: index
        })
    })
