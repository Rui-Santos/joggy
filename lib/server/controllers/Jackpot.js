...
Jackpot.find = function(id, cb) {
    ...
    services.db.collection('jackpots').findOne({ _id: id }, function(err, doc) {
...
Jackpot.prototype.pay = function(user, cb) {
    ...
    services.db.collection('jackpots').update(q, u, function(err, updates) {
        if (err) return cb(err)
...
Jackpot.prototype.contribute = function(satoshi, cb) {
    ...
    services.db.collection('jackpots').update({
        _id: this.model.id
    }, {
