    ...
    onClientHold: function(client, message, callback) {
        ...
        if (vpjob.ranks[rank] === 'royal flush' && coins === 5 && self.jackpot) {
            ...
            db.get('jackpots').pay(
                ...
                function(err, amount) {
                    ...
                    chat.broadcast({ user: 'sprinkles', text: 'YAY!! ' + client.user.get('username') + ' wins the jackpot ' + (amount / 1e5) + '!!!' })
                    self.reset(function() {
                        callback()
                    })
                }
        ...
        db.get('users').give(client.user.id, payout * 1e5, function(err) {
            ...
            if (rank >= 4) {
                ...
                chat.broadcast({
                    user: 'sprinkles',
                    text: 'grats ' + client.user.get('username') + ' with ' + payout + ' for a ' + rankName + '!!' });
            }
