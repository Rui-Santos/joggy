...
_.extend(Faucet.prototype, {
        ...
        db.get('faucet.questions').findOne({}, function(err, question) {
            ...
            db.get('faucet.questions').remove({ _id: question._id }, function(err, removed) {
                ...
                setTimeout(function() {
                    ...
                    chat.broadcast({
                        user: self.user,
                        text: '!!! QUIZ FAIL !!! no one managed to answer my question :-( lets try again in 10 min-ish?'
                    })
                    self.queueNext()
                }, self.timeToAnswer)
            })
    ...
    onChatChat: function(message) {
        ...
        var text = util.format('!!! WINNER !!! %s wins %d CREDITS', message.client.user.get('username'), self.reward / 1e5)
