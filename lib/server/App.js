...
var App = module.exports = function(port) {
    ...
    self.socket.configure(function() {
        self.socket.set('authorization', function(data, cb) {
            ...
            sessionStore.get(sid, function(err, session) {
                ...
                debug('session found')
                debug('session (socket) ' + util.inspect(session))
                cb(null, true)
            })
        })
    })
    ...
    self.chat.on('chat', _.bind(function(message) {
        if (message.client.user.get('username') !== 'abrkn') return
...
_.extend(App.prototype, {
    ...
    getJackpot: function(id) {
        ...
        if (!jackpot) {
            ...
            jackpot.sync = require('./app.db.sync');
    ...
    onSocketConnection: function(socket) {
        ...
        if (socket.handshake.session && socket.handshake.session.user) {
            console.log('existing user is', socket.handshake.session.user)

            var model = new Models.User(socket.handshake.session.user, { parse: true })
            model.sync = require('./app.db.sync')

            client.setUser(model)
    ...
    findOrCreateMachine: function(client, callback) {
        ...
        async.series({
            // todo: look in self.machines... in case of reconnect?
            'find': function(callback) {
                debug('looking for existing machine for user ' + client.user.id)
            ...
            'create': function(callback) {
                if (machineDoc) {
                    debug('found existing machine for user')
                    return callback();
                }
            ...
            var model = new Models.Machine(machineDoc, { parse: true })
            debug('parsed machine')
            model.sync = require('./app.db.sync')
            callback(null, self.getMachine(model))
        })
    }
})